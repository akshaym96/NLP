...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' #added this from https://stackoverflow.com/questions/47068709/your-cpu-supports-instructions-that-this-tensorflow-binary-was-not-compiled-to-u
#-------------------------------------------------------------------------------------------
...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  window_size = 2 * skip_window + 1   # calculating from the formula given in the comments above
  ...
  for i in range(batch_size//num_skips):
    context_count = skip_window
    right = context_count + 1
    left = context_count - 1
    for j in range(num_skips//2):   # considering right word first then left word as described in README
      batch[batch_count] = window[context_count]
      labels[batch_count] = window[right]
      right = right + 1
      batch_count +=1
      batch[batch_count] = window[context_count]
      labels[batch_count] = window[left]
      left = left - 1
      batch_count +=1
    window.append(data[data_index])
    data_index = (data_index +1)%len(data)  # modulus so that it won't go out of bound
  return batch, labels    
